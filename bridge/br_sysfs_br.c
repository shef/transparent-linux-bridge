...
static ssize_t show_multicast_querier(struct class_device *cd, char *buf)
{
	struct net_bridge *br = to_bridge(cd);
	return sprintf(buf, "%d\n", br->multicast_querier);
}

static ssize_t store_multicast_querier(struct class_device *cd,
				       const char *buf, size_t len)
{
	return store_bridge_parm(cd, buf, len, br_multicast_set_querier);
}
static CLASS_DEVICE_ATTR(multicast_querier, S_IRUGO | S_IWUSR,
...
static struct attribute *bridge_attrs[] = {
	...
	&class_device_attr_multicast_querier.attr,
